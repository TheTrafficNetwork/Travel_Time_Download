...
def username_input(API_Key=None, User_Name=None):
    ...
    while not API_Key:
        try:
            ...
            API_Key = APIKey_df.query(
                "Username==@User_Name"
            )["API_Key"].values.item()
            print(f"{User_Name}'s API Key is {API_Key}.")
        except ValueError:
            print(
                "No Username or multiple instances of Username in .csv."
            )
    return API_Key
...
def base_url_creation():
    ...
    Base_URL = (
        r"https://cr.acyclica.com/datastream/route/csv/time"
    )
...
def ask_input_date(message):
    ...
    while True:
        ...
        if (
            validate_date_format(input_date)
            == "Correct Format"
        ):
            break
        else:
...
def user_date_input():
    ...
    """
    while True:
        start_date_request = ask_input_date(
            "Start date(yyyy-mm-dd): "
        )
        end_date_request = ask_input_date(
            "End date(yyyy-mm-dd): "
        )
        ...
        if start_date_request <= end_date_request:
            ...
            start_date = datetime.strptime(
                start_date_request, "%Y-%m-%d"
            )
            end_date = datetime.strptime(
                end_date_request, "%Y-%m-%d"
            )
            break
        else:
            print(
                "Starting date is after Ending date. Please try again.\n"
            )
    return (
        start_date,
        end_date,
...
def start_end_times():
    ...
    requests. 
    """
    StartDate, EndDate, StartDate_Str, EndDate_Str = (
        user_date_input()
    )
    ...
    StartEpochSeconds = int(
        (StartDateInEpoch).total_seconds()
    )
...
def route_dict():
    ...
    try:
        ...
        print(
            "The .csv file containing routes cannot be found. Please check file location"
        )
    return RouteDict
...
def download_files(
    SubFolder, StartTime, URL_Base, Days, key, value
):
    """
...
def merge_downloaded_files(
    ...
    MergedFile = pd.concat(
        pd.read_csv(f, index_col=[0, 1, 2, 3, 4, 5])
        for f in CSV_Files
    )
...
def format_new_files(CombinedFileToBeFormatted):
    ...
    df["Timestamp"] = pd.to_datetime(
        df["Timestamp"], unit="ms"
    ).apply("{:%B %A %w %Y-%m-%d %H:%M:%S}".format)
    df.Strengths = pd.to_timedelta(
        df.Strengths, unit="ms"
    ).apply(timedelta_h_m_s)
    df.Firsts = pd.to_timedelta(df.Firsts, unit="ms").apply(
        timedelta_h_m_s
    )
    df.Lasts = pd.to_timedelta(df.Lasts, unit="ms").apply(
        timedelta_h_m_s
    )
...
def main():
    ...
    StartEpoch, Days, StartDateStr, EndDateStr = (
        start_end_times()
    )
